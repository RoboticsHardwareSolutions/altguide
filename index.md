...
- **/template** -  шаблон директории проекта для удобства проектирования в соответствии с гайдом
- **/utils** - скрипты для автоматизированной проверки соответствия репозитория гайдам, а также общие проверки исходных файлов (помогает избежать досадных ошибок при отправке на производство, встраивается в CI для автоматизации этих процессов )
...
```embed-js
PATH: "vault://attach/example.gitignore"
```


Готовый файл который можно просто положить в корень репозитория находится в  `template/.gitignore`
...
* Рекомендуемый типоразмер пассивных SMD компонентов не мельче 0603
...
* `/doc/` - cодержит документацию относящуюся к проекту  (datasheet "**если** **нада**" вставлять в файл README.md в виде ссылок c корпоративного NAS/OneDrive/YandeksDisk ....)
	...
	* `/pcb/` - папка содержит документацию для печатной платы 
		...
		* `Draftsman.PDF` - Монтажная схема - схема или сборочный чертеж, содержащий позиционные обозначения элементов и их ориентацию. 
		* `board_options.txt` - файл содержит информацию о печатной плате 5 пунктов (толщина платы, толщина меди, количество слоев, цвет паяльной маски, цвет шелкографической маркировки, тип покрытия)
		* `Bill of Materials-xxxxxx.csv` - файлы BOM (если имеются дополнительные внешние разъёмы или прочая фурнитура необходимо указать отдельным файлом ext_bom.csv)
	...
	* `/pinout/` - содержит реалистичное изображение печатной платы с указанием распиновки разъемов в `pinout.png`.  Файл требуется рисовать используя Draw.io и сохранять как редактируемый `.png` файл.       
* `/hardware/` - содержит проект печатной платы `__.PrjPCB __.PcbDoc __.SchDoc` файлы.  (только файлы относящиеся к проекту. Файлы и папки истории библиотеки и прочий мусор не должны лежать в репозитории, CAM файлы и папки Preview , OutputLogs удалить).  
### Файл job

Файл `job.outjob` - содержит готовый шаблон для генерации всех требуемый файлов документации автоматически, будьте внимательны требуется незначительное редактирование в каждом отдельном случае в зависимости от количества слоев печатной палаты и прочих параметров. Готовый файл который можно просто положить в корень репозитория находится в  `files/Job.OutJob`
...
Файл `README.md` - который необходимо размещать в корне репозитория имеет типовое содержание и не нуждается в редактировании в случае использования типовой структуры репозитория. Требуется лишь скачать файл и добавить в репозиторий - **обязательно изменить имя проекта в файле**. 

Пример содержимого README
```embed-js 
PATH: "vault://attach/README.md"
```
...
```embed-js 
PATH: "vault://attach/board_options.txt"
```


Если сомневаетесь в синтаксической правильности написания файла `board_options.txt` - поверьте с помощью онлайн валидатора [тут](https://www.toml-lint.com)    
В ситуации когда плата не является типовой (изготавливается попарным прессованием или имеет не стандартные толщины ) требуется указать в поле type = "custom" и приложить в папку doc/pcb файл - `board_options_draw.png`. Пример такого файла ниже:

![](attach/board_option_draw.png)
...
4. Когда файл будет сохранен, можно будет сгененрировать на его основе .pdf файл из `job.outjob`
...
Столбец **Connect to** - содержит УГО разъема на плате к которому присоединяется коннектор

Пустой шаблон файла `template/ext_bom.csv`
...
![[Screenshot 2025-07-06 at 18.44.25.png]]

![[Screenshot 2025-07-06 at 18.44.56.png]]
...
[board_options]
    ...
    solder_point = 1254            # количество точек пайки
...
## Согласование длин дорожек на печатной плате
Согласование длин дорожек критически важно при проектировании высокоскоростных интерфейсов на печатной плате. Пример из BareMetal PLC XL: QSPI, SWD, RMII и др.
Суть в добавлении плавных изгибов для удлинения коротких дорожек, чтобы длины всех дорожек интерфейса укладывались в необходимый допуск.
![[Pasted image 20260120104907.png]]

### Пошаговая инструкция по согласованию длин дорожек
1. Необходимо создать NetClass для необходимых дорожек (рекомендация)
На этапе разработки принципиальной схемы нужно объединить группу дорожек в NetClass. При успешном действии в свойствах дорожки появится по параметр "Net Class Name". Делается с помощью инструмента Parameter Set. Ниже показаны два варианта:
1 вариант
![[Pasted image 20260120115035.png]]
2 вариант
Тут Parameter Set применен к Blanket
![[Pasted image 20260120110617.png]]

2. Необходимо создать xSignal Class на основе предыдущего
На этапе трассировки печатной платы нужно выделить ножки, образующие нужную линию передачи. Далее ПКМ/xSignals/Create xSignal from selected pins. 
![[Pasted image 20260120113516.png]]
После действия нужно зайти в панель PCB вкладка xSignals. Созданные таким способом xSignals лежат в All Signals. Далее ПКМ/Add Class, выбор нужных линий передачи.
![[Pasted image 20260120112633.png]]
Можно создать xSignals Class другим способом. Например через Design/xSignals/Create xSignals.

3. Необходимо создать правило в MatchedLengths
Это находится в Rules/High Speed/MatchedLengths. 
Cозданное правило MatchedLengths отслеживается при Design Rule Check
![[Pasted image 20260120113302.png]]

4. Добавление "змеек" инструментом Length Tuning
Выбор инструмента Interacltive Length Tuning -> Размещение "змейки" на дорожке -> Галочка Clip to Target, чтобы змейка была минимально короткой и подстраивалась под правило, созданное в предыдущем пункте.
![[Pasted image 20260120114030.png]]
Теперь во вкладке xSignal Class можно наблюдать как все дорожки укладываются в допуск
![[Pasted image 20260120114540.png]]
